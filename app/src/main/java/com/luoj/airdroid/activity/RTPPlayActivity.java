        ...
        btnBack.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (null != remoteSocketServer) {
                    String order = EventInput.getString(1, event.getAction(), KeyEvent.KEYCODE_BACK, 0, 0);
...
//            logd("get complete data." + length);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (null != remoteSocketServer) {
            String order = EventInput.getString(0, event.getAction(), 0, event.getX() / deviceWidth, event.getY() / deviceHeight);
            remoteSocketServer.send(order);
        }
